...
> ⚠️ [!CAUTION] После сброса роутер потеряет IP-адрес, и зайти в него через браузер (WebFig) не получится. Используйте WinBox и подключайтесь по MAC-адресу (он отображается во вкладке Neighbors).
